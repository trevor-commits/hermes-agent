...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    reasoning_config = getattr(agent, "reasoning_config", None)
    overrides = sess.get("resume_runtime_overrides") or {}
    if agent is None and "reasoning_effort" not in mirror:
        reasoning_config = sess.get("create_reasoning_override")
    ...
    model_override = sess.get("model_override") or {}
    info: dict = {
        "model": pending_model or mirror.get("model") or getattr(agent, "model", "") or model_override.get("model") or _resolve_model(),
        "provider": pending_provider or mirror.get("provider") or getattr(agent, "provider", "") or model_override.get("provider") or "",
