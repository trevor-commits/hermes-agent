...
def _pop_session_by_id(sid: str, *, predicate: Callable[[dict], bool] | None = None) -> dict | None:
    ...
    with _sessions_lock:
        ...
        if session is None:
            return None
        # Queue claims take history_lock. Defer cleanup instead of waiting
        # under the registry lock, which resume acquires in the other order.
...
def _interrupt_session_turn(sid: str, session: dict, *, request_id: str | None = None) -> bool:
    """Apply explicit ``session.interrupt`` to a claimed session; return whether compute-host control was used."""
