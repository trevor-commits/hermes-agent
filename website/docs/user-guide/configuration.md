...
- `ws_orphan_reap_grace_s` — how long an idle WS-detached session waits before cleanup. Active turns and delegated work continue across disconnects and can reattach when the client returns. Cleanup waits for completion; explicit Stop and session close still cancel work. (`HERMES_TUI_WS_ORPHAN_REAP_GRACE_S` remains as an internal override.)
