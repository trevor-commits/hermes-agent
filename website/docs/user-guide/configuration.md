...
Codex Responses requests stream internally. Each received SSE event refreshes their stale timer, including reasoning and tool-call events. Healthy streams can therefore continue beyond the stale window. First-byte and event-idle watchdogs still recover silent connections. There is no default total-duration cutoff; an existing positive `HERMES_CODEX_HARD_TIMEOUT_SECONDS` override still imposes an explicit total request deadline.
...
- `ws_orphan_reap_grace_s` — how long an idle WS-detached session waits before cleanup. Active turns and delegated work continue across disconnects and can reattach when the client returns. Initialization, pending input, queued prompts, and workers finishing a turn are also protected by disconnect, TTL, and capacity cleanup. Sessions owned by a separate compute host retain their return route until explicitly closed, because the serving process cannot prove that background work in the host has ended. Explicit Stop and session close still cancel work. (`HERMES_TUI_WS_ORPHAN_REAP_GRACE_S` remains as an internal override.)
