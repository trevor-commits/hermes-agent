...
class AIAgent:
    ...
    def _emit_status(self, message: str, *, kind: str = "lifecycle") -> None:
        ...
        it through ``status_callback(kind, ...)``.  ``kind`` is keyword-only so
        existing callers keep the lifecycle default while special statuses can
        carry structured metadata instead of relying on user-visible text.
    ...
    def _persist_session(
        ...
        Returns ``True`` only when the canonical SQLite flush completed. JSON
        logging and cleanup still run on every path, but they are not enough to
        authorize continuity-sensitive session rollover.
        ...
        def _persist_and_drain() -> bool:
            ...
            db_persisted = self._flush_messages_to_session_db(
    ...
    def _flush_messages_to_session_db_unlocked(
        ...
        try:
            ...
            if _batch_rows:
                ...
                from agent.turn_context import bind_current_turn_durability_receipt
