...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        async def _action(**kw) -> None:
            ...
            self._telegram_typing_server_backoff_chats.discard(str(chat_id))
            if initial_attempt:
                logger.info(
                    "[%s] Initial Telegram typing acknowledgement sent in %.3fs",
                    self.name, asyncio.get_running_loop().time() - attempt_started,
                )
        try:
            ...
            log_typing_failure("[%s] Failed to send Telegram typing indicator: %s", self.name, _redact_telegram_error_text(e), exc_info=True)
    ...
    def _build_message_event(self, message: Message, msg_type: MessageType, update_id: Optional[int] = None) -> MessageEvent:
        ...
        return MessageEvent(
            ...
            reply_to_message_id=reply_to_id, reply_to_text=reply_to_text, auto_skill=topic_skill, auto_skill_route=_skill_route,
