    ...
    for _token, job_id, fire_owner, profile_home in active_fires:
        ...
        try:
            with use_cron_store(profile_home):
                if mark_job_run(
                    job_id, False, reason, expected_fire_owner=fire_owner) is not False:
                    marked.append(job_id)
        except Exception as e:
...
def _resolve_job_runtime(job: dict, job_id: str, jc: _CronJobConfig) -> tuple[dict, str]:
    ...
    try:
        ...
        if not (is_auth or is_transient_net) or _job_has_explicit_inference_pin(job):
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    setup.fallback_model = None if _job_has_explicit_inference_pin(job) else get_fallback_chain(_cfg) or None
...
def _construct_cron_agent(AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir, session_id, session_db):
    ...
    total_cap = _resolve_cron_tool_result_total_max_chars(job)
    agent = AIAgent(
        model=setup.model,
    ...
    agent.tool_result_total_max_chars = total_cap
    return agent
...
def run_job(
    ...
    budget = evaluate_context_parts([prompt], hard_ceiling_tokens=resolve_hard_context_ceiling_tokens())
    if budget.exceeded:
        reason = (
            f"assembled runtime context is ~{budget.estimated_tokens:,} estimated tokens, "
            f"at or above the {budget.usable_tokens:,}-token usable context budget "
            f"(80% of the {budget.hard_ceiling_tokens:,}-token hard ceiling); "
            "provider construction was skipped"
        )
        logger.warning("Job '%s' (ID: %s): BLOCKED by runtime context budget — %s", job_name, job_id, reason)
        return _blocked_config_result(job, reason)
    ...
    try:
        ...
        budget_notice = _tool_output_budget_notice(agent)
        if budget_notice:
            output += f"\n## Tool Output Budget\n\n{budget_notice}\n"
...
def _finish_completed_run(d: _RunDelivery, fire_owner: Optional[str], execution_id: str, *, adapters=None, loop=None) -> bool:
    ...
    # mark_job_run returns False when unmarked, a pause-reason str on new auto-pause, else None
    # (keeper contract) — so "not marked" below must only treat False, not a reason string, as lost.
    ...
    if isinstance(marked, str) and marked.strip():
        ...
        try:
            _pause_delivery_error = _deliver_result(
                job,
                f"⏸️ Cron '{job.get('name') or job['id']}' {marked}. "
...
def _run_one_job_body(
    ...
    try:
        ...
        return _finish_completed_run(d, fire_owner, execution_id, adapters=adapters, loop=loop)
