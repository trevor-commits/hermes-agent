...
def _load_cron_skill_parts(job: dict, skill_names: list[str]) -> tuple[list[str], list[str]]:
    ...
    def _scan_block(name: str, content: str) -> tuple[str, str]:
        from tools.cronjob_prompt_scan import _CRON_SOURCE_CARD_BENIGN_DECEPTIONS, _scan_cron_skill_assembled

        benign = _CRON_SOURCE_CARD_BENIGN_DECEPTIONS if normalize_skill_lookup_name(name) == "source-card-intake" else ()
        cleaned, error = _scan_cron_skill_assembled(content, benign_deception_phrases=benign)
        if error:
            raise _sched.CronPromptInjectionBlocked(error)
        scan = cleaned
    ...
    for skill_name in skill_names:
        ...
        if bundle_key:
            bundle_payload = build_bundle_invocation_message(
                bundle_key, user_instruction="", task_id=task_id, include_skill_segments=True)
            if bundle_payload:
                if parts:
                    parts.append("")
                    scan_parts.append("")
                ...
                scan_parts.append("\n\n".join(
                    _scan_block(name, block)[1] for name, block in bundle_payload[3]
                ))
            else:
        ...
        content, scan_content = _scan_block(skill_name, str(loaded.get("content") or "").strip())
        if parts:
            parts.append("")
            scan_parts.append("")
        header = f'[IMPORTANT: The user has invoked the "{skill_name}" skill, indicating they want you to follow its instructions. The full skill content is loaded below.]'
        parts.extend([header, "", content])
        scan_parts.extend([header, "", scan_content])
    ...
    if skipped:
        ...
        scan_parts.insert(0, parts[0])
    return parts, scan_parts
...
def _build_job_prompt(
    ...
    parts, scan_parts = _load_cron_skill_parts(job, skill_names)
    stable_prefix = None
    if prompt:
        ...
        scan_parts.append("")
        append_user_instruction(scan_parts, prompt)
...
def _scan_assembled_cron_prompt(
    ...
    user_prompt: Optional[str] = None, assembled_for_scan: Optional[str] = None,
) -> str:
    ...
    cannot permanently kill a job); every loose-tier invocation also scans ``user_prompt`` STRICT.
    ...
    from tools.cronjob_prompt_scan import _scan_cron_skill_assembled, _strip_invisible_unicode
    if has_skills or has_injected_data:
        ...
        cleaned, scan_error = _scan_cron_skill_assembled(scan_target)
        assembled = cleaned if assembled_for_scan is None else _strip_invisible_unicode(assembled)[0]
