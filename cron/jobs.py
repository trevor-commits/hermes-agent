...
def _normalize_tool_result_max_chars(value: Any) -> int | None:
    ...
    if (
        ...
        or value < MIN_TOOL_RESULT_MAX_CHARS
        or value > MAX_TOOL_RESULT_MAX_CHARS
    ):
        raise ValueError(
            "tool_result_max_chars must be an integer between "
...
_CREATE_FIELD_NORMALIZERS: Dict[str, Callable[[Any], Any]] = {
    "tool_result_max_chars": _normalize_tool_result_max_chars,
    "tool_result_total_max_chars": _normalize_tool_result_total_max_chars,
...
def create_job(
    ...
    # Check size before the bounded lifecycle scan so oversized jobs get an actionable error.
    ...
    for key, value in (
        ...
        ("tool_result_max_chars", f["tool_result_max_chars"]),
        ("tool_result_total_max_chars", f["tool_result_total_max_chars"]),
    ):
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def apply(jobs, i, job):
        ...
        # Permit edits that shrink an oversized legacy job and all pause/repair actions.
        if not updated.get("no_agent") and {"prompt", "skill", "skills"}.intersection(updates):
            previous_estimate = estimate_job_first_turn_tokens(job)
            try:
                _validate_job_first_turn_size(updated)
            except ValueError:
                if estimate_job_first_turn_tokens(updated) >= previous_estimate:
                    raise
...
def mark_job_run(
    ...
    *,
    ...
    """Return False when unmarked, a pause reason on new auto-pause, otherwise None.
    ...
    def apply(jobs, _i, job):
        ...
        save_jobs(jobs)
        return None if job.get("state") == "completed" else auto_pause_reason
