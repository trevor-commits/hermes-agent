...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    with _jobs_lock():
        ...
        for i, job in enumerate(jobs):
            ...
            if "tool_result_max_chars" in updates:
                updates["tool_result_max_chars"] = _normalize_tool_result_max_chars(
...
def mark_job_run(
    ...
    *,
    ...
    """Mark a job as run, fencing stale completions behind the fire claim.

    Return contract (three-state):
      * ``False``   — not marked: the job was not found, the fire claim was
        lost to another owner (stale completion discarded), or the per-job
        lock could not be acquired.
      * ``None``    — marked, no auto-pause.
      * ``str``     — marked AND auto-paused; the string is the paused_reason.
    """
    with _fire_job_lock(job_id) as acquired:
        if not acquired:
            return False
