...
from hermes_state_common import (
    _COMPRESSION_LOCK_ROW_SQL, _ENDED_ROW_SQL, _RESET_END_REASONS, _RESET_END_REASONS_SQL, _ended_by_continuation,
...
class SessionMessagesMixin:
    ...
    def _check_transcript_write_guards(self, conn, session_id: str, compression_lock_holder: Optional[str],
        ...
        if _ended_by_continuation(conn.execute(_ENDED_ROW_SQL, (session_id,)).fetchone()) and not allow_closed_compression_parent:
    ...
    def replace_messages(self, session_id: str, messages: List[Dict[str, Any]], active_only: bool = False,
        ...
        def _do(conn):
            ...
            elif _ended_by_continuation(conn.execute(_ENDED_ROW_SQL, (session_id,)).fetchone()):
                raise CompressionSessionClosedError(session_id)
            if archive_dropped:
