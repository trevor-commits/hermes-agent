...
// execing Hermes.
function withRemoteUpdateMutex(command, mutexPathWord) {
  const script = `
...
`.trim()
  ...
  // expandRemotePath already supplies a quoted shell word, including $HOME
  // expansion. Quoting it again would lock a different, literal path.
  return `python3 -c ${shq(script)} ${mutexPathWord} ${shq(command)}`
}
