...
for (const homeStyle of ['absolute', 'tilde'] as const) {
  test.skipIf(process.platform === 'win32')(
    `spawn locks the updater mutex at the expanded ${homeStyle} home`,
    async () => {
      const directory = await mkdtemp(path.join(os.tmpdir(), 'hermes-update-mutex-path-'))
      const hermesPath = path.join(directory, 'hermes')
      const home = homeStyle === 'absolute' ? path.join(directory, 'home') : '~/.hermes'
      const mutexHome = homeStyle === 'absolute' ? home : path.join(directory, '.hermes')
      ...
      try {
        ...
        await exec(command, { shell: '/bin/bash', cwd: directory, env: { ...process.env, HOME: directory } })

        assert.equal(await readFile(path.join(mutexHome, '.hermes-update-in-progress.mutex'), 'utf8'), '')
      } finally {
