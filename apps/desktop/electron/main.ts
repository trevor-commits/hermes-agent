...
function setPoolLimits(raw) {
  ...
  void evictLruPoolBackends(poolMaxBackends()).catch(error => {
    rememberLog(`Profile backend activity probe failed: ${error instanceof Error ? error.message : String(error)}`)
  })
  startPoolIdleReaper()
...
// Keep update ownership until the owner exits or releases its marker.
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    isCancelled: () => backendShutdown.hasStarted()
  })
...
function startPoolIdleReaper() {
  ...
  poolIdleReaper = setInterval(() => {
    void reapIdlePoolBackends().catch(error => {
      rememberLog(
        `Profile backend idle activity probe failed: ${error instanceof Error ? error.message : String(error)}`
      )
    })
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  // silently for background profiles — so we only log while parked.
  {
    ...
    await waitForUpdateClearance(updateGateDeps(), {
      ...
      pollMs: UPDATE_WAIT_POLL_MS,
      isCancelled: () => backendShutdown.hasStarted() || backendPool.get(poolKey) !== entry || backendPool.get(poolKey) !== entry
    })
