...
/// so the desktop's launch gate can detect a dead owner and self-heal.
/// Elapsed time is diagnostic: a slow live updater still owns the checkout.
///
...
/// This legacy Option reader cannot report marker I/O errors. Never panics.
///
...
/// Probe errors preserve ownership unless they positively identify a dead PID.
fn pid_probe_error_preserves_owner(error: Option<i32>) -> bool {
    #[cfg(windows)]
    {
        error != Some(windows_sys::Win32::Foundation::ERROR_INVALID_PARAMETER as i32)
    }
    #[cfg(not(windows))]
    {
...
/// True when a positive PID exists or its liveness cannot be determined.
#[cfg(windows)]
fn pid_is_alive(pid: u32) -> bool {
    ...
    unsafe {
        ...
        if handle.is_null() {
            return pid_probe_error_preserves_owner(Some(GetLastError() as i32));
        }
        let mut code: u32 = 0;
...
#[cfg(not(windows))]
fn pid_is_alive(pid: u32) -> bool {
    // Do not turn oversized PID data into a negative process-group probe.
    ...
    // Signal zero delivers nothing. Only ESRCH proves the owner is dead.
    let rc = unsafe { libc::kill(pid, 0) };
    if rc == 0 {
        return true;
    }
    pid_probe_error_preserves_owner(std::io::Error::last_os_error().raw_os_error())
}
...
impl UpdateMarkerGuard {
    ...
    fn acquire(path: PathBuf) -> Result<Self, MarkerOwner> {
        ...
        if let Some(owner) = live_marker_owner(&path) {
            if owner.pid == pid {
                ...
                // claim verbatim so retries retain the original elapsed time.
    ...
    /// pid holding a marker — which blocks desktop startup and every
    /// other updater. Idempotent: `Drop` still runs
    /// and tolerates an already-removed marker.
    fn complete(&self) {
        if !self.owned || !marker_owned_by_self(&self.path) {
            return;
        }
...
mod tests {
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        ...
        // every in-app desktop update loop forever. Preserve the holder's
        // original age for diagnostics when adopting it.
    ...
    #[test]
    fn acquire_refuses_a_live_owner_past_twenty_minutes() {
        let dir = unique_tmp_dir("marker-stale-age");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join(".hermes-update-in-progress");

        // A slow update still owns the checkout after twenty minutes.
        let mut foreign = spawn_foreign_holder();
        let foreign_pid = foreign.id();
        ...
        let original = format!("{foreign_pid}\n{long_ago}");
        std::fs::write(&marker, &original).unwrap();

        let result = UpdateMarkerGuard::acquire(marker.clone());
        let retained = std::fs::read_to_string(&marker).unwrap();
        let _ = foreign.kill();
        let _ = foreign.wait();
        let owner = result.err().expect("a live old owner must prevent concurrent update");
        assert_eq!(owner.pid, foreign_pid);
        assert!(owner.age_secs >= 21 * 60);
        assert_eq!(retained, original, "the owner's original marker is preserved");
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn completed_update_preserves_a_handoff_partners_marker() {
        let dir = unique_tmp_dir("marker-complete-handoff");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join(".hermes-update-in-progress");
        let guard = UpdateMarkerGuard::acquire(marker.clone())
            .unwrap_or_else(|_| panic!("no live owner => acquire must succeed"));
        let replacement = "4294967294\n123\n";
    ...
    #[test]
    fn pid_probe_uncertainty_preserves_a_positive_owner() {
        assert!(pid_probe_error_preserves_owner(None));
        #[cfg(windows)]
        {
            use windows_sys::Win32::Foundation::{ERROR_ACCESS_DENIED, ERROR_INVALID_PARAMETER};
            assert!(pid_probe_error_preserves_owner(Some(ERROR_ACCESS_DENIED as i32)));
            assert!(!pid_probe_error_preserves_owner(Some(ERROR_INVALID_PARAMETER as i32)));
        }
        #[cfg(not(windows))]
        {
            assert!(pid_probe_error_preserves_owner(Some(libc::EPERM)));
            assert!(pid_probe_error_preserves_owner(Some(libc::EIO)));
            assert!(!pid_probe_error_preserves_owner(Some(libc::ESRCH)));
        }
