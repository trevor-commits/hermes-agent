...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    for name, sql_type in (("owner_pid", "INTEGER"), ("owner_started_at", "INTEGER"), ("task_json", "TEXT"),
                           ...
                           ("work_key", "TEXT"), ("work_kind", "TEXT"), ("delivery_mode", "TEXT")):
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        for row in rows:
            ...
            event = {
                ...
                **{k: task[k] for k in (*_ROUTING_KEYS, "work_key", "work_kind", "delivery_mode") if task.get(k)}}
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    work_key: str = "", work_kind: str = "", delivery_mode: str = "",
) -> Dict[str, Any]:
    ...
    record: Dict[str, Any] = {
        ...
        "work_key": str(work_key or ""), "work_kind": str(work_kind or ""),
    ...
    # Capacity, durable reservation, and worker acceptance are one boundary.
    # Replays can only observe a duplicate after the original was scheduled.
    with _records_lock:
        if work_key:
            for existing in _records.values():
                if existing.get("work_key") == work_key:
                    return {"status": "duplicate", "delegation_id": existing["delegation_id"]}
        ...
        live_units = 1 + sum(1 for r in _records.values() if r.get("status") in _LIVE_STATES)
        try:
            # Units can share a slot; size by units so a new group starts promptly.
            executor = _get_executor(max(max_async_children, live_units))
        except Exception as exc:
            return {"status": "rejected", "error": f"Failed to initialize async delegation executor: {exc}"}
        try:
            existing_id = _persist_dispatch(record)
        except Exception:
            logger.exception("Could not persist async delegation %s before scheduling", delegation_id)
            return {"status": "rejected", "error": "Durable delegation state could not be recorded; worker not started."}
        ...
        try:
            ...
            try:
                ...
                logger.exception("Could not remove unscheduled async delegation %s", delegation_id)
                return {"status": "rejected", "error": "Worker scheduling failed and durable cleanup was incomplete; no retry was started."}
...
def dispatch_async_delegation(
    *, goal: str, context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str],
    ...
    handle = _dispatch(
        ...
        work_key=work_key, work_kind=work_kind, delivery_mode=delivery_mode,
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    evt = {
        ...
        **{k: record[k] for k in (*_ROUTING_KEYS, "work_key", "work_kind", "delivery_mode") if record.get(k)},
