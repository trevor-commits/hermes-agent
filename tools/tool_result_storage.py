...
def maybe_persist_tool_result(content: str, tool_name: str, tool_use_id: str, env=None,
                              config: BudgetConfig = DEFAULT_BUDGET,
                              threshold: int | float | None = None, *,
                              return_receipt: bool = False) -> str | ToolResultPersistence:
    ...
    location succeeds."""
    def _result(value: str, persisted: bool = False):
        receipt = ToolResultPersistence(value, persisted)
    ...
    def _persisted(path: str, host_suffix: str = "") -> str | ToolResultPersistence:
        logger.info("Persisted large tool result: %s (%s, %d chars -> %s%s)",
                    tool_name, tool_use_id, len(content), path, host_suffix)
        return _result(_build_persisted_message(preview, has_more, len(content), path), True)
