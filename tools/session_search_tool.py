...
def _read_session(db, session_id: str, head: int = 20, tail: int = 10, link_profile: str = None) -> str:
    ...
    if not meta:
        ...
        if hint:
            return tool_error(f"session_id not found: {session_id}", success=False, chat_source_hint=hint)
        return tool_error(f"session_id not found: {session_id}", success=False)
    def _edges():
        if hasattr(db, "get_message_edges"):
            return db.get_message_edges(session_id, head=head, tail=tail)
        rows = db.get_messages(session_id)
        total = len(rows)
        truncated = total > head + tail
        return {"rows": rows[:head] + rows[-tail:] if truncated else rows, "total": total, "truncated": truncated}
    view, err = _loud(_edges, "session edge view failed for %s: %s", "failed to load session", session_id)
    if err:
        return err
    shaped = [_shape_message(m) for m in view["rows"]]
    total, truncated = view["total"], view["truncated"]
    logging.debug("session_search read session_id=%s total=%s returned=%s truncated=%s elapsed_ms=%.3f",
                  session_id, total, len(shaped), truncated, (time.perf_counter() - started) * 1000)
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        " For an exact Claude/Codex/Cursor/Hermes/Copilot chat ID, run "
        "`/Users/gillettes/.codex/scripts/chat-source describe <id>` first."
    ),
    "parameters": {
        "type": "object",
        "properties": {
