...
def run_preflight_gate(
    ...
    _last_preflight_pressure: Any, current_input_too_large: bool = False,
    ...
    hard_config = int(getattr(agent, "_hard_ceiling_tokens", 0) or 0)
    context_length = int(getattr(_compressor, "context_length", 0) or 0)
    v._hard_ceiling_tokens = _preflight_threshold
    if hard_config > 0:
        ceiling = min(hard_config, context_length) if context_length > 0 else hard_config
        v._hard_ceiling_tokens = max(ceiling, _preflight_threshold)
