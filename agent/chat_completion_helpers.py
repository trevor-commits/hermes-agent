...
def _codex_wait_notice_recovery(*, stale_timeout: float, ttfb_enabled: bool, ttfb_timeout: float,
    ...
    idle_timeout: float, idle_requires_progress: bool, elapsed: float, hard_timeout: float = 0.0) -> str:
    ...
    if math.isfinite(stale_timeout):
        ...
        last_activity = max(0.0, activity_ts - call_start) if activity_ts is not None else 0.0
...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    # Healthy stream events rearm the stale window; total elapsed work is not
    # silence. Only an explicit operator deadline limits the whole request.
    hard_timeout = (env_float("HERMES_CODEX_HARD_TIMEOUT_SECONDS", 0.0)
                    if codex and openai_codex_backend else 0.0)
    ...
    return _NonStreamWatchdogs(stale_timeout=stale_timeout, codex=codex, est_tokens=est_tokens, hard_timeout=hard_timeout,
