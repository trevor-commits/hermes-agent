...
def _summarize_user_message(user_message: str) -> str:
    ...
    return _strip_image_attachment_notes(strip_control_wrappers(user_message if described is None else described))
...
def is_titleable_user_message(user_message: str) -> bool:
    ...
    return (isinstance(user_message, str) and bool(user_message.strip()) and not user_message.lstrip().startswith(_MACHINE_PREFIXES)
            and (bool(_summarize_user_message(user_message).strip()) or _is_image_only_opener(user_message)))
...
def derive_title(user_message: str) -> Optional[str]:
    ...
    return line or ("Image" if _is_image_only_opener(user_message) else None)
...
def maybe_auto_title(
    ...
    if not _summarize_user_message(user_message).strip():
        return  # Image-only title needs no model call. (keeper guard)
