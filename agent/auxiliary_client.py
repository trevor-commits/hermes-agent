...
class _CodexStreamGuard:
    ...
    def close_attempt_stream(self, failure_note: str) -> None:
        """Wake this attempt; only its owner may release the response's FDs."""
        with self._attempt_stream_lock:
        ...
        try:
            ...
            network_stream = getattr(response, "extensions", {}).get("network_stream")
            sock = _socket_from_stream(network_stream)
            if sock is not None:
    ...
    def _close_client_on_timeout(self) -> None:
        ...
        if threading.get_ident() == self._owner_tid:
            ...
            # An SDK response owns transport FDs too. Wake its reader with shutdown;
            # release_stream() performs the real close when the owner unwinds.
