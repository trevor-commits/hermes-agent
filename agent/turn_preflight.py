...
class PreflightGateVerdict:
    ...
    _hard_ceiling_tokens: int = 0
    _hard_ceiling_reason: Optional[str] = None
...
def run_preflight_compression(
    ...
    if (
        ...
        if v.messages is _pre_api_input and (
            ...
            if v.messages is not _pre_api_input:
                v.conversation_history, _ = rebind_turn_after_compression(
            ...
            v.api_call_count = _refund_api_call(agent, v.api_call_count)
            handoff_message_count = len(v.messages)
            if _should_skip_model_call_for_reference_handoff(
                v.messages, user_message, turn_identity=getattr(agent, "_persist_user_turn_identity", None)
            ):
                ...
                return _done("break")
            if len(v.messages) != handoff_message_count:
                v.conversation_history, _ = rebind_turn_after_compression(
                    agent, v.messages, v.conversation_history, rebaseline=False
                )
            return _done("continue")
    ...
    if provider_overflow_preflight:
        ...
        return _done("return", _exhausted_result())
    v._hard_ceiling_reason = _hard_context_ceiling_reason(
        agent, estimated_tokens=request_pressure_tokens, ceiling_tokens=v._hard_ceiling_tokens,
        compression_attempts=v.compression_attempts, max_compression_attempts=max_compression_attempts,
        preflight_compression_blocked=v._preflight_compression_blocked,
        compression_cooldown=_compression_cooldown,
        preflight_deferred=bool(defer_preflight(request_pressure_tokens)),
    ...
    if (
        ...
        if messages is _post_tool_input and compression_skipped_due_to_lock(agent):
            ...
            if messages is not _post_tool_input:
                conversation_history, _ = rebind_turn_after_compression(agent, messages, conversation_history)
            handoff_message_count = len(messages)
            if _should_skip_model_call_for_reference_handoff(
            ...
            if len(messages) != handoff_message_count:
                conversation_history, _ = rebind_turn_after_compression(
                    agent, messages, conversation_history, rebaseline=False
                )
