...
def _reanchor(agent: Any, messages: List[Any], user_message: Any) -> int:
    ...
    idx = reanchor_current_turn_user_idx(messages, user_message, turn_identity=getattr(agent, "_persist_user_turn_identity", None))
...
def _idle_compaction(
    ...
    if not (agent.compression_enabled and not out.current_input_too_large and _idle_after > 0 and messages):
        return
    ...
    if out.messages is not messages:
        ...
        out.conversation_history, out.current_turn_user_idx = rebind_turn_after_compression(
...
def _preflight_compression(
    ...
    if out.current_input_too_large:
        return
...
def _run_preflight_passes(
    ...
    for _pass in range(_max_preflight_passes):
        ...
        if out.messages is _preflight_input and compression_skipped_due_to_lock(agent):
            ...
            break
        if out.messages is not _preflight_input:
            out.compressed = True
            from agent.turn_context import rebind_turn_after_compression
            out.conversation_history, out.current_turn_user_idx = rebind_turn_after_compression(
                agent, out.messages, out.conversation_history
