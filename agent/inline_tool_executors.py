...
def _session_search(agent, args: dict, ctx: InlineToolContext) -> Any:
    if "profile" in args:
        ...
        if not isinstance(profile, str):
            return json.dumps({"success": False, "error": "profile must be a valid profile identifier"})
        try:
            from hermes_cli.profiles import validate_profile_name
            validate_profile_name(profile)
        except ValueError:
            return json.dumps({"success": False, "error": f"profile {profile!r} must be a valid profile identifier"})
        session_db = None
    else:
        profile = ""
    ...
    return _call_tool(
        ...
        profile=profile, db=session_db, current_session_id=agent.session_id,
    )
