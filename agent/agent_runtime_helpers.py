...
def _merge_consecutive_users(messages: List[Dict]) -> Tuple[List[Dict], int]:
    ...
    for msg in messages:
        ...
        if (
            ...
            from agent.turn_context import CURRENT_TURN_IDENTITY_KEY, carry_current_turn_identity
            if prev.get(CURRENT_TURN_IDENTITY_KEY) or msg.get(CURRENT_TURN_IDENTITY_KEY):
                prev = dict(prev)
                merged[-1] = prev
            prev["content"] = (
