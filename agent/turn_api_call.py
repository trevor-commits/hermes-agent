...
class ApiCallVerdict:
    ...
    interrupted: Any
    api_call_count: int
    _deterministic_trim_attempts: int
...
def perform_api_call(
    ...
    interrupted: Any, messages: Any = None, conversation_history: Any = None,
    api_messages: Any = None, request_pressure_tokens: int = 0,
    _hard_ceiling_reason: Optional[str] = None, _hard_ceiling_tokens: int = 0,
    _deterministic_trim_attempts: int = 0,
) -> ApiCallVerdict:
    """Issue the request (see ``_should_stream`` for the streaming decision)."""
    response = None
    from agent.conversation_loop import (
        _HARD_CONTEXT_CEILING_BLOCKED, _compression_deferred_result,
        _hard_context_ceiling_result, _provider_request_tokens_rough,
    )
    hard_ceiling_trip = (
        (request_pressure_tokens, _hard_ceiling_reason)
        if _hard_ceiling_reason == "input_too_large" else None
    )
    ...
    def _verdict(action: str, result: Optional[Dict[str, Any]] = None) -> ApiCallVerdict:
        return ApiCallVerdict(
            ...
            _deterministic_trim_attempts=_deterministic_trim_attempts, result=result,
        )
    ...
    def _perform_api_call(next_api_kwargs):
        ...
        if _hard_ceiling_reason:
            terminal_estimate = _provider_request_tokens_rough(next_api_kwargs)
            if terminal_estimate >= _hard_ceiling_tokens:
                hard_ceiling_trip = (terminal_estimate, _hard_ceiling_reason)
    ...
    if hard_ceiling_trip is not None:
        thinking_spinner = stop_thinking_spinner(agent, thinking_spinner)
        terminal_estimate, terminal_reason = hard_ceiling_trip
        compressor = agent.context_compressor
        deficit = terminal_estimate - _hard_ceiling_tokens
        if (
            terminal_reason not in {"compression_lock", "input_too_large"}
            and _deterministic_trim_attempts < int(getattr(compressor, "max_deterministic_attempts", 0) or 0)
            and 0 <= deficit <= int(getattr(compressor, "deterministic_trim_max_deficit_tokens", 0) or 0)
        ):
            ...
            reclaimed = truncate_oversized_tool_results(
                api_messages, reclaim_chars=deficit * 4 + 128, mirror=messages,
            )
            if reclaimed > 0:
                logger.info("Last-mile deterministic trim reclaimed %d chars; retrying provider send", reclaimed)
                # Reuse the reserved call after a free trim. Refunding here would undercount
                # the successful retry or refund twice if the terminal guard still blocks it.
        ...
        try:
            ...
            pass
        result = (
            _compression_deferred_result(agent, messages, api_call_count)
            if terminal_reason == "compression_lock"
            else _hard_context_ceiling_result(
                agent, messages, conversation_history, api_call_count,
                estimated_tokens=terminal_estimate, ceiling_tokens=_hard_ceiling_tokens,
