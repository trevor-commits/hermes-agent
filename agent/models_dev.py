...
def fetch_models_dev(force_refresh: bool = False, *, allow_network: bool = True, allow_disk: bool = True) -> Dict[str, Any]:
    ...
    With network disabled, ``allow_disk=False`` also avoids opening the cache on a stalled volume."""
    global _models_dev_cache, _models_dev_cache_time, _models_dev_retry_after
    if not allow_network:
        if allow_disk and not _models_dev_cache and (disk_data := _load_disk_cache()):
...
def _registry_provider(mdev_id: str, allow_network: bool, allow_disk: bool = True) -> Optional[Dict[str, Any]]:
    ...
    registry = (fetch_models_dev() if allow_network else fetch_models_dev(allow_network=False)) if allow_disk else fetch_models_dev(allow_network=allow_network, allow_disk=False)
    provider_data = registry.get(mdev_id)
    return provider_data if isinstance(provider_data, dict) else None


def _registry_models(mdev_id: str, *, allow_network: bool, allow_disk: bool = True) -> Optional[Dict[str, Any]]:
    """The ``models`` dict of a models.dev provider entry, or None."""
    provider_data = _registry_provider(mdev_id, allow_network, allow_disk)
    models = provider_data.get("models", {}) if provider_data is not None else None
    return models if isinstance(models, dict) else None


def _get_provider_models(provider: str, *, allow_network: bool = False, allow_disk: bool = True) -> Optional[Dict[str, Any]]:
    ...
    return _registry_models(mdev_id, allow_network=allow_network, allow_disk=allow_disk) if mdev_id else None
...
def lookup_models_dev_context(provider: str, model: str, *, allow_network: bool = False, allow_disk: bool = True) -> Optional[int]:
    ...
    if override_ctx is not None:
        return override_ctx
    models = _get_provider_models(provider, allow_network=allow_network, allow_disk=allow_disk)
