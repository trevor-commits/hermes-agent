...
def _apply_run_tool_output_budget(
    ...
    *,
    ...
    """
    full_output_persisted = full_content_already_persisted

    def _result(emitted: Any) -> Any:
    ...
    with lock:
        ...
        def _persist_complete() -> None:
            nonlocal full_output_persisted
            if (
                full_content is None
                or env is None
                or full_output_persisted
            ):
                return
            ...
            full_output_persisted = (
                isinstance(persistence, ToolResultPersistence)
...
def _prepare_text_tool_result_for_context(
    ...
    *,
    ...
    budget_persistence = _apply_run_tool_output_budget(
        agent,
        emitted,
    ...
    if isinstance(budget_persistence, ToolResultPersistence):
        receipt = budget_persistence
    else:
        receipt = ToolResultPersistence(
            content=budget_persistence,
            full_output_persisted=full_output_already_persisted,
        )
    ...
    *,
    ...
    if not _is_multimodal_tool_result(function_result):
        persistence = _prepare_text_tool_result_for_context(
            agent, function_result, tool_name=function_name, tool_use_id=tool_call_id,
            env=get_active_env(effective_task_id), persistence_config=budget,
            subdir_hints=subdir_hints, return_receipt=True,
        )
        if not isinstance(persistence, ToolResultPersistence):
            persistence = ToolResultPersistence(persistence, False)
        persisted_result = persistence.content
    ...
    if persistence is not None and persistence_receipts is not None:
        persistence_receipts[id(tool_message)] = _receipt_for_final_model_content(persistence, tool_message["content"])
...
def _finalize_tool_batch(agent, messages: list, effective_task_id: str, num_tools: int, budget: BudgetConfig, persistence_receipts: dict[int, ToolResultPersistence] | None = None) -> None:
    ...
    if num_tools <= 0:
        return
    enforce_turn_budget(messages[-num_tools:], env=get_active_env(effective_task_id), config=budget, persistence_receipts=persistence_receipts)
...
def _append_batch_results(agent, messages: list, effective_task_id: str, batch: _ConcurrentBatch, budget: BudgetConfig, persistence_receipts: dict[int, ToolResultPersistence] | None = None) -> bool:
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True, persistence_receipts: dict[int, ToolResultPersistence] | None = None) -> None:
    ...
    if not _append_batch_results(agent, messages, effective_task_id, batch, _tool_budget, persistence_receipts):
        return
    if finalize:
        _finalize_tool_batch(agent, messages, effective_task_id, len(parsed_calls), _tool_budget, persistence_receipts)
...
def _publish_sequential_result(agent, messages: list, ref: _ToolCallRef, managed: _ManagedToolResult, *, tool_duration: float, index: int, budget: BudgetConfig, persistence_receipts: dict[int, ToolResultPersistence] | None = None) -> bool:
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True, persistence_receipts: dict[int, ToolResultPersistence] | None = None) -> None:
    ...
    for i, tool_call in enumerate(tool_calls, 1):
        ...
        if not _publish_sequential_result(agent, messages, ref, managed, tool_duration=tool_duration, index=i, budget=_tool_budget, persistence_receipts=persistence_receipts):
            return
    ...
    if finalize:
        _finalize_tool_batch(agent, messages, effective_task_id, len(tool_calls), _tool_budget, persistence_receipts)
...
def execute_tool_calls_segmented(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, segments=None) -> None:
    ...
    persistence_receipts: dict[int, ToolResultPersistence] = {}
    if segments is None:
    ...
    for kind, calls in segments:
        ...
        run_segment(agent, segment_message, messages, effective_task_id, api_call_count, finalize=False, persistence_receipts=persistence_receipts)
        if getattr(agent, "_incremental_persistence_failed", False):
            return

    total_tools = len(assistant_message.tool_calls)
    if total_tools > 0:
        _finalize_tool_batch(agent, messages, effective_task_id, total_tools, _budget_for_agent(agent), persistence_receipts)
