...
def build_bundle_invocation_message(
...
) -> Optional[Tuple[str, List[str], List[str]] | Tuple[str, List[str], List[str], List[Tuple[Optional[str], str]]]]:
    ...
    that check — gateway passes it, None resolves from env). When requested, a fourth
    item contains the header and each rendered block paired with its canonical skill name.
    ...
    *None*, the platform resolves from session env vars and the global disabled list still applies. Mirrors
    ...
    if include_skill_segments:
        return (*result, [(None, header), *zip(loaded_names, skill_blocks)])
    return result
