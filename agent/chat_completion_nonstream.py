...
class _NonStreamRequest:
    ...
    def _emit_wait_notice(self, elapsed: float, *, heartbeat: bool = True) -> None:
        ...
        try:
            ...
            recovery = h._codex_wait_notice_recovery(stale_timeout=wd.stale_timeout,
                ...
                elapsed=elapsed, hard_timeout=wd.hard_timeout)
    ...
    def _stale_kill(self, elapsed: float, timeout_limit: float) -> None:
        """No response within the stale timeout: kill and count toward the
        circuit breaker (#58962, see ``_stale_streak``)."""
        agent, wd = self.agent, self.wd
        silent_hint = h._codex_silent_hang_hint(agent, self.api_kwargs)
        h._report_stale_nonstream_kill(agent, self.api_kwargs, elapsed, timeout_limit, hint=silent_hint)
        self._abort_request("stale_call_kill")
        h._bump_stale_streak(agent)
        h._touch_stale_kill_activity(agent, elapsed)
        self._await_worker_after_kill(
            f"Non-streaming API call timed out after {int(elapsed)}s with no response (threshold: {int(timeout_limit)}s)"
            + (f". {silent_hint}" if silent_hint else ""))
    ...
    def run(self):
        ...
        while t.is_alive():
            ...
            if (retry_started_ts is None and wd.idle_enabled and idle_elapsed is not None
                ...
                break
            # Valid stream events (or a reconnect) rearm this request's stale
            # window. Keep the separately configured absolute deadline intact.
            activity_ts = retry_started_ts if retry_started_ts is not None else last_event_ts
            stale_elapsed = now - activity_ts if activity_ts is not None else elapsed
            hard_deadline_hit = wd.hard_timeout > 0 and elapsed > wd.hard_timeout
            timeout_elapsed = elapsed if hard_deadline_hit else stale_elapsed
            timeout_limit = wd.hard_timeout if hard_deadline_hit else wd.stale_timeout
            if timeout_elapsed > timeout_limit:
                self._stale_kill(timeout_elapsed, timeout_limit)
                break
