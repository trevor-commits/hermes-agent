...
from agent.turn_context import CURRENT_TURN_IDENTITY_KEY, CurrentTurnDurabilityReceipt, stable_message_content_digest
...
def _restore_user_after_reference_handoff(
    messages: List[Dict[str, Any]], user_message: Any, *, turn_identity: Optional[str] = None
) -> bool:
    ...
    if isinstance(last, dict) and last.get("role") == "user" and last.get("content") == user_message:
        return False
    restored = {"role": "user", "content": user_message}
...
def _should_skip_model_call_for_reference_handoff(
    ...
    return reference_handoff_would_drive_next_model_call(messages) and not (
        _restore_user_after_reference_handoff(messages, user_message, turn_identity=turn_identity)
    )
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    if stored_cwd and stored_cwd != str(current_cwd):
        return False
    if current_cwd.resolve() == Path.home().resolve() and any(
...
_CTX_FIELDS = frozenset({
    ...
    "active_system_prompt", "current_turn_user_idx", "current_input_too_large", "_preflight_compression_blocked",
})
...
def _run_phase(fn, agent, state: _LoopState, **extra):
    ...
    # Compaction rebinds the persistence anchor immediately in its shared helper.
    # Mirror that anchor before the next phase assembles API-only user context.
    anchor = getattr(agent, "_persist_user_message_idx", None)
    if type(anchor) is int:
        state.current_turn_user_idx = anchor
    return verdict
...
def _run_api_retry_loop(agent, s: _LoopState) -> Optional[Dict[str, Any]]:
    ...
    while s.retry_count < s.max_retries:
        ...
        try:
            ...
            call = _run_phase(perform_api_call, agent, s)
            if call.action == "return":
                return call.result
            if call.action == "continue":
                continue
            if call.action == "break":
                return None
...
def run_conversation(
    ...
    while (s.api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        s._deterministic_trim_attempts = 0
