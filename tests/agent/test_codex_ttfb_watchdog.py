...
def test_active_codex_stream_survives_long_elapsed_time(monkeypatch, input_size, hard_timeout):
    ...
    from agent import chat_completion_helpers as h
    from agent import chat_completion_nonstream as nonstream

    requests = []
    original_init = nonstream._NonStreamRequest.__init__

    def record_request(self, *args, **kwargs):
        original_init(self, *args, **kwargs)
        requests.append(self)

    monkeypatch.setattr(nonstream._NonStreamRequest, "__init__", record_request)
    ...
    class ActiveStreamThread:
        ...
        def join(self, timeout=None):
            now[0] += 30.0
            self.polls += 1
            state = requests[-1].codex_watchdog_state
            with state.lock:
                state.last_event_ts = now[0]
...
def test_codex_sdk_stream_crosses_stale_window_without_reconnect(tmp_path, monkeypatch):
    ...
    # Leave scheduling headroom while streaming for longer than the stale window.
    monkeypatch.setattr(agent, "_compute_non_stream_stale_timeout", lambda _k: 2.0)
    monkeypatch.delenv("HERMES_CODEX_HARD_TIMEOUT_SECONDS", raising=False)
    try:
