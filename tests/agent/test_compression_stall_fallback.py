...
# The 50ms/200ms fixture budgets could expire before the daemon pool worker
# began on a busy test host. That exercises cancelled queue admission instead
# of the intended started-but-silent provider. Keep actual timeout coverage
# with enough scheduling headroom for the worker to enter the fake provider.
_TEST_IDLE_SECONDS = 0.5
_TEST_CEILING_SECONDS = 5.0


def _run(worker, *, chain, timeouts, messages,
         idle=_TEST_IDLE_SECONDS, ceiling=_TEST_CEILING_SECONDS):
    ...
    try:
        with _patch_chain([CHAIN_ENTRY]):
            msgs, _prompt = run_compress_context_with_progress_timeout(
                ...
                messages=original,
                system_prompt_fallback="degraded-prompt",
                idle_timeout_seconds=_TEST_IDLE_SECONDS,
                total_ceiling_seconds=_TEST_CEILING_SECONDS,
...
def test_fallback_that_also_stalls_degrades_after_one_attempt():
    ...
    timeouts = []
    entry = dict(CHAIN_ENTRY, timeout=_TEST_IDLE_SECONDS)
