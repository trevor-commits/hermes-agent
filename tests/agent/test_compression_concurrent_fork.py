...
def test_review_fork_compacts_oversized_snapshot_in_memory(tmp_path: Path) -> None:
    ...
    def _run_threshold_crossing_review(self, *args, **kwargs):
        ...
        # Leave room for the warm first request's system/tool schemas. The
        # provider usage below then crosses this real pressure budget, without
        # declaring the first request illegal under the local hard ceiling.
        ...
        def _should_compress(_tokens):
            ...
            if _should_compress_calls["count"] == 1:
                assert _tokens >= self.context_compressor.threshold_tokens
                return True
            return False
