...
@pytest.mark.parametrize("platform, expected", [("darwin", 2), ("linux", 1)])
def test_guest_barriers_preserve_platform_durability_when_unset(
    monkeypatch, tmp_path, platform, expected
):
    _config(monkeypatch, {})
    monkeypatch.setattr(hermes_state.sys, "platform", platform)
    conn = sqlite3.connect(tmp_path / "state.db")
    try:
        conn.execute("PRAGMA journal_mode=DELETE")
        conn.execute("PRAGMA synchronous=1")
