...
from tools.computer_use import cua_backend_driver, cua_backend_daemon
...
class TestEmbeddedDaemonOverlayFlag:
    ...
    def test_serve_process_disables_overlay_when_policy_requires_it(self, platform):
        daemon = cua_backend._EmbeddedCuaDaemon("/usr/bin/cua-driver", "unrestricted")
        process = MagicMock()
        process.poll.return_value = None
        status = MagicMock(returncode=0)

        with patch.object(cua_backend.sys, "platform", platform), patch.object(
        ...
        if platform == "darwin":
            assert command[:5] == ["/usr/bin/open", "-n", "-g", "-a", "/Applications/CuaDriver.app"]
        else:
