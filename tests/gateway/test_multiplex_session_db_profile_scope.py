...
def test_optional_rollover_preserves_routes_when_profile_uses_another_database(multiplex_homes):
    root, profile = multiplex_homes
    store = _make_store(root)
    store.config.multiplex_profiles = True
    store._profile_home_cache["fitness"] = profile
    token = set_hermes_home_override(profile)
    try:
        entry = store.get_or_create_session(SessionSource(
            platform=Platform.TELEGRAM, chat_id="profile-chat", profile="fitness",
        ))
        store.append_to_transcript(entry.session_id, {"role": "user", "content": "keep this turn"})
        db = store._db_for_session_id(entry.session_id)
        routing_db = store._routing_db
        assert db is not routing_db
        before = routing_db.load_gateway_routing_entries()
        messages = db.get_messages(entry.session_id)
        result = store.rollover_session_with_carryover(
            entry.session_key, entry.session_id,
            {"role": "user", "content": "carryover", "_compressed_summary": True},
        )
        assert result is None
        assert routing_db.load_gateway_routing_entries() == before
        assert db.get_messages(entry.session_id) == messages
        assert not db.get_session(entry.session_id).get("end_reason")
