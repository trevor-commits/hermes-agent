...
@pytest.mark.asyncio
async def test_gateway_retry_prepares_live_carrier_text_and_keeps_scaffold(
    tmp_path, monkeypatch
):
    import hermes_state
    monkeypatch.setattr(hermes_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
    ...
    event = MessageEvent(text="/retry", message_type=MessageType.TEXT, source=MagicMock())
    assert await gw._prepare_retry_turn(event, session_entry) is None
...
async def test_gateway_retry_does_not_rewind_a_newer_plain_turn(
    ...
    result = await gw._prepare_retry_turn(
...
async def test_gateway_retry_waits_for_alias_rollover_then_rewrites_only_child(
    ...
    def rewrite_only_after_child_rebind(session_id, messages, active_only=False, **kwargs):
        assert session_id == child_id
        assert messages == retry_history[:2]
        assert active_only is True
        assert kwargs["reject_active_turn_lease"] is True
...
@pytest.mark.asyncio
async def test_retry_entry_point_defers_preparation_to_leased_dispatch():
    runner = GatewayRunner.__new__(GatewayRunner)
    event = MessageEvent(text="/retry", message_type=MessageType.TEXT, source=MagicMock())
    runner._handle_message = AsyncMock(return_value="reply")
    assert await runner._handle_retry_command(event) == "reply"
    assert event._gateway_retry_requested is True
