...
@pytest.mark.asyncio
async def test_startup_boot_sends_notifies_home_after_unclean_shutdown(monkeypatch):
    """Crash recovery keeps its home-channel notice inside the bounded send path."""
    monkeypatch.setenv("HERMES_STARTUP_RESTORE_DRAIN_TIMEOUT", "2")
...
@pytest.mark.asyncio
async def test_startup_boot_sends_suppresses_unclean_notice_after_chat_restart(monkeypatch):
    """A targeted /restart completion must not also leak to the home channel."""
    monkeypatch.setenv("HERMES_STARTUP_RESTORE_DRAIN_TIMEOUT", "2")
    ...
    await runner._await_startup_boot_sends(
        planned_restart_notification_pending=False,
        unclean_startup=True,
        chat_restart_notification_pending=True,
    )
