...
class TestInboundMediaAuthorizationGate:
    ...
    async def test_live_media_redacts_long_path_before_bounding(self, tmp_path):
        parent = tmp_path
        private_parts = []
        # Exceed the 900-character redaction bound without exceeding macOS
        # PATH_MAX (1024), including the test runner's temporary directory.
        padding = (950 - len(str(tmp_path)) - len("/handoff.txt")) // 6 - len("/private-0-")
        for index in range(6):
