...
def test_timeout_kill_reaps_setsid_grandchild(tmp_path):
    ...
    try:
        ...
        cmd = f"{_sh_quote(_sys.executable)} -c {_sh_quote(script)} {_sh_quote(str(pid_file))}"
        result = env.execute(cmd, timeout=3)

        # The command must have hit the timeout/kill path.
        assert result.get("returncode") == 124, f"expected timeout, got: {result!r}"
        assert "timed out" in result.get("output", "").lower(), result

        # The grandchild wrote its pid before the kill.
        deadline = time.monotonic() + 5.0
        while time.monotonic() < deadline and not pid_file.exists():
            time.sleep(0.05)
        assert pid_file.exists(), f"grandchild never wrote its pid file: {result!r}"
