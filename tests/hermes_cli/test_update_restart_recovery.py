...
def test_system_daemon_recovery_retains_exact_native_authority():
    plan = SimpleNamespace(runtimes=[_runtime("default", "launchd-system")])
    assert update_cmd._gateway_recovery_partition(plan) == ({"default": "launchd-system"}, [])
    assert update_cmd._gateway_service_matches_profile("default", "ai.hermes.gateway.daemon")
    assert not update_cmd._gateway_service_matches_profile("work", "ai.hermes.gateway.daemon")
    assert update_cmd._gateway_recovery_partition(plan, skip_profiles={"default"}) == ({}, [])
