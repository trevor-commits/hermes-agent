...
class GatewayStartupMixin:
    ...
    async def _await_startup_boot_sends(self, *, planned_restart_notification_pending: bool, unclean_startup: bool = False,
        chat_restart_notification_pending: bool = False,
    ) -> None:
        ...
        *before* ``_finish_startup_restore`` released the gate. See #91969.
        ...
        async def _boot_sends() -> None:
            ...
            if planned_restart_notification_pending:
                ...
                # cleanly. Keep the recovery notice in this bounded send path,
                # and avoid duplicating a targeted /restart completion.
                try:
                    ...
                    logger.warning(
                        "Unclean-shutdown startup notification failed: %s", e
                    )
    ...
    def _start_log_startup_environment(self) -> None:
        """Bind the gateway loop and log the startup environment."""
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        self._clean_startup = _clean_marker.exists()
        if self._clean_startup:
    ...
    async def _start_post_connect_services(self, connected_count: int) -> None:
        ...
        # Arm running-loop checks after blocking startup; retain the native
        # startup watchdog until this handoff is complete.
    ...
    async def _start_finish_wiring(self, connected_count: int) -> None:
        ...
        await self._await_startup_boot_sends(
            ...
            unclean_startup=not getattr(self, "_clean_startup", True),
            chat_restart_notification_pending=chat_restart_notification_pending,
        )
