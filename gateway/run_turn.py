...
def is_context_overflow_failure_result(agent_result: dict, history_len: int) -> bool:
    ...
    if not agent_result.get("failed"):
        return False
    if agent_result.get("compression_exhausted") or (agent_result.get("hard_context_ceiling_blocked") and agent_result.get("agent_persisted")):
        return True
...
class GatewayTurnMixin:
    ...
    async def _hmwa_acquire_turn_lease(self, _quick_key, run_generation, session_entry, _session_env_tokens=None):
        ...
        if refreshed is None or (
            refreshed.session_id != waited_session_id
            and not self._rebind_turn_lease(_quick_key, run_generation, refreshed.session_id)
        ):
            raise TurnLeaseTimeoutError(
                waited_session_id, owner_key=_quick_key, generation=run_generation, wait_seconds=0,
            )
        return refreshed
    ...
    def _hmwa_classify_turn_failure(self, agent_result, history, session_entry):
        ...
        if is_context_overflow_failure:
            logger.info(
                ...
                "failure in session %s (reason=%s estimated=%s) to prevent session growth loop.",
                session_entry.session_id,
                agent_result.get("compression_block_reason") or agent_result.get("error"),
    ...
    async def _hmwa_compression_exhaustion_reset(
        ...
        """Auto-reset only when the agent proves the current turn and its continuity are durable. Never on a lock-contended defer — that is the
        OPPOSITE case (a concurrent path holds the lock and is shrinking it). Returns
        ``(response, session_entry)``."""
        # Missing or legacy rollover evidence fails closed; lock contention is retryable.
        ...
        elif _gateway_rollover_is_authorized(agent_result) and session_entry and session_key:
    ...
    async def _hmwa_persist_turn_transcript(
        ...
        if is_context_overflow_failure:
            ...
            if agent_failed_early or hidden_reasoning_incomplete:
                ...
                # A flushed current-turn row already owns this input even when it lacks a
                # platform ID. Repeating it would invalidate the exact durability receipt.
                turn_start = agent_result.get("history_offset", len(history))
                turn_rows = (agent_messages[turn_start:]
                             if isinstance(agent_messages, list) and isinstance(turn_start, int)
                             and 0 <= turn_start <= len(agent_messages) else [])
                content = _user_row.get("content")
                agent_already_flushed = isinstance(content, str) and any(
                    isinstance(row, dict) and row.get("role") == "user"
                    and row.get("_db_persisted") is True and not row.get("_compressed_summary")
                    and isinstance(row.get("content"), str)
                    and (row["content"] == content or row["content"].endswith(content))
                    for row in turn_rows
                )
                if agent_already_flushed or (
                    event.message_id and await store.has_platform_message_id(sid, str(event.message_id))
                ):
                    logger.info(
    ...
    async def _hmwa_prepare_turn(self, event, source, session_entry, session_key, _quick_key, run_generation):
        ...
        try:
            ...
            # Journal the exact turn before consuming its recoverable auto-skill claim.
            active_turn_marked = await self._mark_durable_active_turn(event, session_entry.session_key)
            if getattr(event, "_gateway_retry_requested", False):
                retry_error = await self._prepare_retry_turn(event, session_entry)
                if retry_error is not None:
                    setattr(event, "_gateway_skip_goal_continuation", True)
                    return retry_error, _session_env_tokens
            from gateway.run_source_card import _is_source_card_intake_event
            if _is_source_card_intake_event(event, source):
                return (await self._handle_source_card_intake_event(
                    event, source, session_entry, active_turn_marked,
                )), _session_env_tokens
            _auto = getattr(event, "auto_skill", None)
            _auto_original_text = event.text
            _auto_payload_ready = False
            if active_turn_marked:
                _auto_payload_ready = self._apply_claimed_auto_skill(
                    event,
                    _quick_key,
                    session_entry.session_key,
                )
            if active_turn_marked and _auto_payload_ready:
                try:
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str, run_generation: int):
        ...
        try:
            ...
            # All turn writes succeeded; rollover may now commit its continuation.
            try:
                if (
