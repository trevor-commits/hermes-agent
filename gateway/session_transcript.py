                ...
                if isinstance(exc, CompressionSessionClosedError):
                    ...
                    if child_id:
                        ...
                        with self._transcript_retry_lock:
                            _ack_head()
                        logger.error(
                            "Session DB transcript append rejected for continuation-ended %s with "
    ...
    def rewind_session(
        ...
        *,
        ...
        with self._get_transcript_drain_lock():
            ...
            for attempt in range(2):
                try:
                    target_session_id = self._resolve_transcript_mutation_target_serialized(
                        candidate_session_id
                    )
                    expected_active_ids = db.get_active_message_ids(target_session_id)
                    ...
                    if attempt == 0:
                        continue
                    logger.warning("rewind_session: continuation-ended target %s", exc.session_id)
                    return None
                except Exception as e:
    ...
    def _resolve_transcript_mutation_target_serialized(
        ...
        for _ in range(100):
            ...
            if (
                ...
                break
            child = db.find_live_compression_child(current)
