...
class BasePlatformAdapter(ABC):
    ...
    async def _start_typing_refresh(self, event: MessageEvent, interrupt_event: asyncio.Event,
        ...
        first_tick_done = None
        if self._accepts_kwarg(self._keep_typing, "first_tick_done", var_kw=False, unknown=True):
            first_tick_done = asyncio.Event()
            kwargs["first_tick_done"] = first_tick_done
        task = asyncio.create_task(self._keep_typing(event.source.chat_id, **kwargs))
        if first_tick_done is not None:
            try:
                await asyncio.wait_for(first_tick_done.wait(), timeout=1.75)
            except asyncio.TimeoutError:
                logger.warning("[%s] Initial typing acknowledgement did not finish within 1.75s; continuing message processing", self.name)
        return task
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        _thread_metadata = _thread_metadata_for_event(event)
        typing_task = await self._start_typing_refresh(event, interrupt_event, _thread_metadata)
        try:
