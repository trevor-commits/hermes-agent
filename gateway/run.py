...
from gateway.run_source_card import GatewaySourceCardMixin
...
def _start_gateway_make_shutdown_signal_handler(runner, _signal_initiated_shutdown: list):
    ...
    def shutdown_signal_handler(received_signal=None):
        ...
        if planned_takeover:
            runner._exit_reason = runner._exit_reason or f"planned --replace takeover ({_signame})"
            logger.info("Received %s as a planned --replace takeover — exiting cleanly", sig_name or "SIGTERM")
        elif planned_stop:
            runner._exit_reason = runner._exit_reason or f"planned gateway stop ({_signame})"
            ...
            runner._exit_reason = runner._exit_reason or f"external signal ({_signame})"
