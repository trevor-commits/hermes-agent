    ...
    if is_macos():
        if get_system_launchd_gateway_plist_path().exists():
            system_probe = (
                ...
                if all_profiles else _probe_system_launchd_gateway
            )
            _loaded, system_pid, _output = system_probe()
        ...
        # launchd owns stderr_timestamp, whose gateway is a direct child.
        # Protect both from the stale/manual sweep and orphan reaper.
        ...
        if pid is not None:
            ...
            if _graceful_restart_via_sigusr1(service_pid, wait_budget):
                ...
                if _wait_for_launchd_service_pid(
                    label, service_pid, timeout=15.0, domain=domain
                ):
                    print("✓ Service restart requested")
                    _clear_launchd_unsupported_marker()
                    return
