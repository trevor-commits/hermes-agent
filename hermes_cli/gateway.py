...
def _stderr_wrapper_gateway_children(pid: int) -> set[int] | None:
    """Resolve the native logger's gateway children without signalling it.

    None means the PID is not a recognized wrapper. An empty set means it
    is a wrapper but no gateway child could be verified; never send that
    wrapper SIGUSR1, whose default action would terminate the logger.
    """
    try:
        import psutil

        process = psutil.Process(pid)
        if process.cmdline()[1:3] != ["-m", "hermes_cli.stderr_timestamp"]:
            return None
...
def _system_daemon_identity_details(plist_path: Path | None = None) -> dict:
    ...
    details["install_matches"] = belongs_to_install(venv) or any(
        belongs_to_install(arg) for arg in argv
    )
...
def _drain_or_stop_system_gateway(pid: int, wait_budget: float) -> bool:
    """Keep busy drains; bound recovery only for a confirmed target wedge."""
    children = _stderr_wrapper_gateway_children(pid)
    if children is not None and len(children) != 1:
        return False
    gateway_pid = next(iter(children)) if children else pid
    daemon_home = _system_daemon_identity_details().get("daemon_home")
    if daemon_home and probe_gateway_loop_liveness(
        gateway_pid, home=Path(daemon_home)
    ) == GATEWAY_LOOP_WEDGED:
        print(f"⚠ System gateway PID {gateway_pid} event loop is unresponsive — bounded recovery...")
        if not _escalate_wedged_gateway(gateway_pid):
            return False
        return gateway_pid == pid or _wait_for_pid_exit(pid, 10.0)
    return _graceful_restart_via_sigusr1(pid, wait_budget)
...
def restart_system_launchd_gateway_for_update() -> bool:
    ...
    if not _drain_or_stop_system_gateway(pid, _get_restart_exit_wait_budget()):
        return False
    return _wait_for_system_daemon_pid_for_install(pid)
...
def _restart_system_launchd_gateway() -> None:
    ...
    if not _drain_or_stop_system_gateway(daemon_pid, wait_budget):
...
def _system_launchd_status(deep: bool = False) -> None:
    ...
    if not identity["matches"]:
        ...
        print("✗ System gateway daemon is installed but not loaded")
        print("  System-domain service recovery is required; the user LaunchAgent remains retired.")
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    if is_macos():
        if get_system_launchd_gateway_plist_path().exists():
            probe = _probe_system_launchd_gateway_for_install if all_profiles else _probe_system_launchd_gateway
            _loaded, system_pid, _output = probe()
        ...
        # launchd may supervise the logger; protect its verified gateway children too.
        for service_pid in tuple(pids):
            pids.update(_stderr_wrapper_gateway_children(service_pid) or ())
    return pids
...
def _graceful_restart_via_sigusr1(pid: int, drain_timeout: float, *, on_progress=None) -> bool:
    ...
    Args:
        ...
        drain_timeout: Seconds to wait for the process to exit after sending
            ...
            ``resolve_restart_exit_wait_budget(...)``.
        on_progress: Zero-arg callback run on every poll so a long wait can
            report what the gateway is still holding for
            (``update_cmd_drain_report``).
    ...
    if pid <= 0:
        return False
    gateway_children = _stderr_wrapper_gateway_children(pid)
    if gateway_children is not None and len(gateway_children) != 1:
        return False
    signal_pid = next(iter(gateway_children)) if gateway_children else pid
    try:
        os.kill(signal_pid, signal.SIGUSR1)  # windows-footgun: ok — POSIX signal, guarded by hasattr(signal, 'SIGUSR1') above
...
def launchd_restart():
    ...
    try:
        pid = get_running_pid()
        # PID files identify the gateway; launchd identifies its logger.
        # Capture that parent before draining so fresh-service verification
        # cannot mistake the unchanged wrapper for a replacement gateway.
        service_pid = pid
        parent_pid = _get_parent_pid(pid) if pid is not None else None
        if parent_pid and pid in (_stderr_wrapper_gateway_children(parent_pid) or ()):
        ...
        if pid is not None:
            ...
            from hermes_cli.update_cmd_drain_report import drain_progress_reporter
            if _graceful_restart_via_sigusr1(service_pid, wait_budget, on_progress=drain_progress_reporter(budget_s=wait_budget)):
                ...
                if _wait_for_launchd_service_pid(label, service_pid, timeout=15.0, domain=domain):
                    _launchd_ok("✓ Service restart requested")
                    return
...
def launchd_status(deep: bool = False):
    ...
    if foreign_system:
        print("ℹ A same-label system daemon exists for another Hermes home; it is not this profile owner.")
...
def _is_service_running() -> bool:
    ...
    if is_macos() and _system_daemon_blocks_current_profile():
        loaded, daemon_pid, _output = _probe_system_launchd_gateway()
        return loaded and daemon_pid is not None
    if is_macos() and _launchd_gateway_service_installed():
        try:
...
def _installed_service_kind_for(windows) -> str | None:
    ...
    if _systemd_unit_installed():
        return "systemd"
    if is_macos() and (_system_daemon_blocks_current_profile() or get_launchd_plist_path().exists()):
        return "launchd"
