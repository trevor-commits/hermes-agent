...
class CLIChatTurnMixin:
    ...
    def _chat_stage_user_message(self, agent, message):
        ...
        with persist_lock if persist_lock is not None else contextlib.nullcontext():
            ...
            identity_session = getattr(agent, "session_id", None) or getattr(self, "session_id", None) or "session"
            current_input_identity = f"{identity_session}:cli:{uuid.uuid4().hex}"
            staged_user_message[CURRENT_TURN_IDENTITY_KEY] = current_input_identity
