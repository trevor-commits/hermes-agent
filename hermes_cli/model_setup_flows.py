...
from hermes_cli.model_setup_flows_common import (
    ...
    _ensure_dict_section, _ensure_flow_api_key, _finish_model, _retained_credential_allows_endpoint,
...
def _prompt_base_url_override(effective_base: str, base_url_env: str, credential_base: str = "", *, persist_env: bool = True) -> str | None:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    existing_key, _resolved, abort, credential_base = _ensure_flow_api_key(
        "openrouter", pconfig, missing_hint=("Get one at: https://openrouter.ai/keys", ""))
    if abort:
        return

    effective_base = credential_base or OPENROUTER_BASE_URL
    ...
    selected = _prompt_model_selection(
        ...
        confirm_base_url=effective_base, confirm_api_key=_resolved or existing_key)
...
def _model_flow_kimi(config, current_model=""):
    """Kimi / Moonshot model selection; retained keys keep their endpoint; new keys use the prefix:
    ...
    _, existing_key, abort, credential_base = _ensure_flow_api_key(provider_id, pconfig)
    if abort:
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    current_base = credential_base or _env_base_url(base_url_env)
    if not current_base:
    ...
    if credential_base and all(
        credential_base.rstrip("/") != _stepfun_base_url_for_region(key).rstrip("/") for key, _ in regions
    ):
...
def _gemini_tier_ok(existing_key: str, pconfig, base_url_env: str, credential_base: str = "") -> bool:
    ...
    tier = probe_gemini_tier(existing_key, credential_base or _env_base_url(base_url_env) or pconfig.inference_base_url)
    if tier == "free":
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "opencode-free":
        ...
        if abort:
            return
    if provider_id == "gemini" and existing_key and not _gemini_tier_ok(existing_key, pconfig, base_url_env, credential_base):
        return
    ...
    if provider_id == "zai":
        ...
        effective_base = _prompt_base_url_override(effective_base, base_url_env, credential_base, persist_env=provider_id != "actual")
