...
_RESTART_MECHANISMS = {
    "systemd": "systemd", "launchd": "launchd", "launchd-system": "launchd-system", "desktop": "desktop",
...
_MECHANISM_DESCRIPTIONS = {
    ...
    "launchd-system": "system LaunchDaemon drain + fresh-PID verification",
    ...
    with _probe("Gateway-state inventory"):
        ...
        for profile, home in profile_homes:
            ...
            if sock is not None:
                ...
                if sup in {"launchd", "external"}:
                    detected = supervisor(pid)
...
def _gateway_named_in(r: RuntimeRecord, names: set) -> bool:
    ...
    if r.supervisor == "launchd-system":
        ...
        return get_system_launchd_gateway_label() in names
    if r.supervisor == "launchd":
        suffix = "" if r.profile == "default" else f"-{r.profile}"
        return f"ai.hermes.gateway{suffix}" in names
...
def match_runtime_outcomes(
    ...
    try:
        ...
        for r in plan.runtimes:
            # The pre-pull plan survives module reload: old class identity is not authoritative.
            if all(hasattr(r, name) for name in ("kind", "profile", "pid", "supervisor", "restart_via")):
                outcomes.append(
