...
def _detect_supervisor_for_pid(
    ...
    try:
        ...
        if is_macos():
            ...
            if loaded and system_pid is not None:
                from hermes_cli.gateway import _stderr_wrapper_gateway_children

                if system_pid == pid or pid in (_stderr_wrapper_gateway_children(system_pid) or ()):
        ...
        for profile, home in profile_homes:
            ...
            if identity:
                ...
                if sock_pid is not None:
                    ...
                    # A wrapped gateway can only declare "external"; its
                    # launchd provenance lives on the logger parent. Refine
                    # generic declarations using the verified service tree.
                    if supervisor in {"launchd", "external"}:
                        detected = _detect_supervisor_for_pid(
                            sock_pid, service_pids, windows_service_pids
                        )
                        if detected in {"launchd", "launchd-system"}:
                            supervisor = detected
...
def match_runtime_outcomes(
    ...
    *,
    ...
    try:
        ...
        for runtime in plan.runtimes:
            # The plan predates the pull and module-cache purge. Its records
            # belong to the old RuntimeRecord class, so class identity would
            # silently drop the whole plan precisely during a real update.
            if not all(hasattr(runtime, name) for name in (
                "kind", "profile", "pid", "supervisor", "restart_via"
            )):
                continue
            r = runtime
            if r.supervisor == "launchd-system":
                from hermes_cli.gateway import get_system_launchd_gateway_label

                runtime_units = {get_system_launchd_gateway_label()}
            elif r.supervisor == "launchd":
                suffix = "" if r.profile == "default" else f"-{r.profile}"
                runtime_units = {f"ai.hermes.gateway{suffix}"}
            else:
                runtime_units = {
                    unit for unit in failed_set | restarted_set
                    if r.profile in unit
                    or (r.profile == "default" and "hermes-gateway" in unit)
                }
            outcome = "unaccounted"
            if r.profile in relaunched or r.profile in external:
                ...
                outcome = "stopped"
            elif runtime_units & failed_set:
                outcome = "failed"
            elif runtime_units & restarted_set:
                outcome = "restarted"
            outcomes.append(
                {
                    "kind": r.kind,
