...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str, str]:
    """Resolve an API-key token, source, and its paired endpoint in one read."""
    ...
    if key_env:
        key, shared_url = get_env_credential(key_env, provider=provider_id)
        val = _usable_declared_secret(provider_id, key, key_env)
        if val:
            return val, key_env, shared_url

    for env_var in pconfig.api_key_env_vars:
        key, shared_url = get_env_credential(env_var, provider=provider_id)
        val = _usable_declared_secret(provider_id, key, env_var)
        if val:
    ...
    try:
        ...
        if pool and pool.has_credentials():
            ...
            for entry in candidates:
                ...
                if val:
                    return val, pool_source, getattr(entry, "runtime_base_url", "") or ""
    except Exception:
        pass
    return "", "", ""
...
def _env_secret(name: str, provider: str = "") -> bool:
    from agent.credential_pool import get_env_prefer_dotenv
    return has_usable_secret(get_env_prefer_dotenv(name, provider=provider))
...
def _explicit_env_credentials_present(normalized: str) -> bool:
    ...
    if pconfig.auth_type == "api_key":
        return any(_env_secret(v, normalized) for v in pconfig.api_key_env_vars if v not in _IMPLICIT_ENV_VARS)
...
def _pool_entry_is_explicit(entry: Any, provider: str = "") -> bool:
    ...
    if source.startswith("env:"):
        ...
        return bool(env_var and _env_secret(env_var, provider))
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    env_url = credential_url or _provider_env_base_url(pconfig)
