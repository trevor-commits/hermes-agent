...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        ...
        known.update(item.slug for item in provider_catalog())
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if provider:
        with warn_on_error(""):
            ...
            accept.add(normalize_provider(provider))
    ...
    if provider and provider != "auto" and (catalog_provider is None and not (accept & valid_provider_ids)):
