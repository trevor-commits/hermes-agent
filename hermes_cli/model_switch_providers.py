...
def _shared_env_present(env_vars, provider: str) -> bool:
    from agent.credential_pool import get_env_prefer_dotenv
    return any(get_env_prefer_dotenv(name, provider=provider) for name in env_vars)
...
def _raw_pool_usable(hermes_id: str, *, for_picker: bool = False) -> bool:
    try:
        ...
        return _credential_pool_is_usable(
            hermes_id, raw_pool_present=bool(read_credential_pool().get(hermes_id)), for_picker=for_picker)
    except Exception:
        return False
...
def _overlay_has_env_creds(pid: str, hermes_slug: str, overlay, read_env) -> bool:
    ...
    from agent.credential_pool import get_env_prefer_dotenv
    read_env = lambda name: get_env_prefer_dotenv(name, provider=hermes_slug)
    has_creds = False
...
def _collect_authed_provider_slugs(
    ...
    for hermes_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _shared_env_present(env_vars, hermes_id) or _raw_pool_usable(hermes_id):
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    for hermes_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        if not (_shared_env_present(env_vars, hermes_id) or _raw_pool_usable(hermes_id, for_picker=b.for_picker)):
            continue
...
def _overlay_has_creds(b: _PickerBuild, pid: str, hermes_slug: str, overlay) -> bool:
    ...
    if not has_creds:
        ...
        try:
            has_creds = _credential_pool_is_usable(hermes_slug, for_picker=b.for_picker)
        except Exception as exc:
