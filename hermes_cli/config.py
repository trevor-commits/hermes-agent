...
def _coerce_config_set_value(key: str, value: str) -> Any:
    ...
    if key == "delegation.max_iterations":
        try:
            return normalize_delegation_max_iterations(value)
        except ValueError as exc:
            _exit_invalid(f"✗ Cannot set '{key}': {exc}")
    default_value = _default_value_for_key(key)
    if isinstance(default_value, list):
        try:
            parsed = fast_safe_load(value)
        except Exception as exc:
            _exit_invalid(f"Cannot set '{key}': expected a YAML/JSON list ({exc}).")
        if not isinstance(parsed, list):
            _exit_invalid(f"Cannot set '{key}': expected a YAML/JSON list, got {type(parsed).__name__}.")
        return parsed
...
def _cmd_config_provenance(args):
    key = getattr(args, 'key', None)
    if not key:
        _usage_exit("Usage: hermes config provenance <key> [--json]", [
            "hermes config provenance compression.enabled",
            "hermes config provenance security.redact_secrets --json"])
...
_CONFIG_SUBCOMMANDS = {
    ...
    "get": _cmd_config_get,
    "provenance": _cmd_config_provenance,
    "source": _cmd_config_provenance,
    "set": _cmd_config_set,
