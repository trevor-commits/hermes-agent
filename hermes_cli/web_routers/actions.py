...
async def update_hermes():
    ...
    with http_failure("Failed to spawn hermes update", 500, "Failed to start update"):
        ...
        args = ["update"] if branch == "main" else ["update", "--branch", branch]
        proc = _spawn_hermes_action(args, "hermes-update", env_overrides={"HERMES_ACTION_ID": action_id})
...
def _recent_upstream_commits(branch: str = "main", n: int = 20) -> List[Dict[str, Any]]:
    """Commits the local checkout is behind ``origin/<branch>`` by, newest first; [] on any failure.
...
async def check_hermes_update(force: bool = False):
    ...
    from hermes_cli.update_branch import branch_pinned_update_command, resolve_update_branch
    ...
    payload: Dict[str, Any] = {
        ...
        "update_command": branch_pinned_update_command(recommended_update_command_for_method(install_method)), "message": None,
    }
