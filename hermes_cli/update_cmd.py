...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    if is_macos():
        ...
        if system_label in ordered:
            print(f"    launchctl print system/{system_label}")
            print("  Recover the installed system LaunchDaemon; do not create a user service.")
        if any(label != system_label for label in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    # The system daemon was handled above. Its retired user plist must not
    # trigger a second restart or a false user-domain supervision failure.
    if not _system_daemon_blocks_current_profile():
...
_FRESH_RESTART_SUPERVISORS = frozenset({"systemd", "launchd", "launchd-system", "service", "s6"})
    ...
    try:
        ...
        try:
            ...
            if pull_result.returncode != 0:
                # Matching branch names do not prove an upstream rewrite:
                # a maintained fork may have committed local changes too.
                # Merge on every divergence; never discard those commits.
                print(
                    f"  ⚠ Fast-forward not possible — merging origin/{branch} "
                    "to preserve local commits..."
                )
