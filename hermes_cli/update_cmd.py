...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    """Merge every divergence; a matching branch name never authorizes discarding local commits."""
    print(f"  ⚠ Fast-forward not possible — merging origin/{branch} to preserve local commits...")
...
def _pull_updates(
    ...
    keep_stash):
    """Fast-forward or merge onto ``origin/<branch>`` and settle the autostash.
    A post-pull syntax error rolls back. Exits on failure; returns pre-pull SHA."""
