...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* invokes Hermes with a profile selector."""
    return _wrapper_profile(path) is not None


def _wrapper_profile(path: Path) -> Optional[str]:
    """Read the profile from a generated shell or batch wrapper, including quoted paths."""
    try:
    ...
    for line in content.splitlines():
        ...
        if args and args[0] == "exec":
            args = args[1:]
        if len(args) >= 3 and Path(args[0]).name == "hermes" and args[1] == "-p":
            canon = normalize_profile_name(args[2])
            if _PROFILE_ID_RE.fullmatch(canon):
                return canon
    return None
